...
All projects follow the same branch-driven CI/CD pattern in BitBucket.
...
Use the project code for the pipeline you are changing (for example `CPS`,
`ECP`, `DMC`, `CNSV`, and so on).
`CPS` is used in commands as an example only; replace it with your assigned
project code.
...
3. CI builds and deploys that project's changes to the DEV environment using
  `FPACDEV` naming.
