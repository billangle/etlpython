WITH vault AS
  (SELECT DISTINCT dm.IRR_PRAC_DURB_ID,
   ...
   WHERE dm.Row_Num_Part = 1 ),
     remainder AS
  (SELECT mart.IRR_PRAC_DURB_ID
  FROM CAR_DM_STG.IRR_PRAC_DIM mart
  JOIN vault
  ON vault.IRR_PRAC_DURB_ID = mart.IRR_PRAC_DURB_ID
  WHERE mart.CUR_RCD_IND = 1
     ...
     AND (COALESCE (vault.IRR_PRAC_DESC,
                    'X') <> COALESCE (mart.IRR_PRAC_DESC,
                                      'X')) )
INSERT INTO CAR_DM_STG.IRR_PRAC_DIM (IRR_PRAC_DURB_ID, CUR_RCD_IND, DATA_EFF_STRT_DT, DATA_EFF_END_DT, CRE_DT, IRR_PRAC_CD, IRR_PRAC_DESC)
SELECT vault.IRR_PRAC_DURB_ID,
       1,
