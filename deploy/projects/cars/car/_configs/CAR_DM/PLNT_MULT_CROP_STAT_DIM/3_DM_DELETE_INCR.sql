...
FROM
  (SELECT DISTINCT DV_DR.DATA_EFF_END_DT,
   ...
   WHERE (DATE (DV_DR.DATA_EFF_END_DT) = TO_TIMESTAMP ('{ETL_DATE}',
          ...
          AND NOT EXISTS
            (SELECT '1'
             FROM edv.PLNT_MULT_CROP_STAT_RS sub
             WHERE sub.PLNT_MULT_CROP_STAT_CD = DV_DR.PLNT_MULT_CROP_STAT_CD
