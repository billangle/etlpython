WITH vault AS
  (SELECT DISTINCT dm.PLNT_SCND_STAT_CD,
   ...
   WHERE dm.Row_Num_Part = 1 ),
     remainder AS
  (SELECT mart.PLNT_SCND_STAT_DURB_ID
    , mart.PGM_YR
  FROM CAR_DM_STG.PLNT_SCND_STAT_DIM mart
  JOIN vault
  ON vault.PLNT_SCND_STAT_DURB_ID = mart.PLNT_SCND_STAT_DURB_ID
     ...
     AND (COALESCE (vault.PLNT_SCND_STAT_NM,
                    'X') <> COALESCE (mart.PLNT_SCND_STAT_NM,
                                      'X')))
INSERT INTO CAR_DM_STG.PLNT_SCND_STAT_DIM (PLNT_SCND_STAT_DURB_ID, CUR_RCD_IND, DATA_EFF_STRT_DT, DATA_EFF_END_DT, CRE_DT, PLNT_SCND_STAT_CD, PGM_YR, PLNT_SCND_STAT_NM)
SELECT vault.PLNT_SCND_STAT_DURB_ID, 
       1, 
