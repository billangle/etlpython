WITH vault AS
  (SELECT DISTINCT dm.RPT_ACRG_MOD_RSN_CD,
   ...
   WHERE dm.Row_Num_Part = 1 ),
     remainder AS
  (SELECT mart.RPT_ACRG_MOD_RSN_DURB_ID 
  FROM CAR_DM_STG.RPT_ACRG_MOD_RSN_DIM mart
  JOIN vault
  ON vault.DURB_ID = mart.RPT_ACRG_MOD_RSN_DURB_ID
     ...
     AND (COALESCE (vault.RPT_ACRG_MOD_RSN_CD,
          ...
          OR COALESCE (vault.RPT_ACRG_MOD_RSN_DESC,
                       'X') <> COALESCE (mart.RPT_ACRG_MOD_RSN_DESC,
                                         'X')))
INSERT INTO CAR_DM_STG.RPT_ACRG_MOD_RSN_DIM (RPT_ACRG_MOD_RSN_DURB_ID, CUR_RCD_IND, DATA_EFF_STRT_DT, DATA_EFF_END_DT, CRE_DT, RPT_ACRG_MOD_RSN_CD, RPT_ACRG_MOD_RSN_DESC)
