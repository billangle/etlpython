INSERT INTO CAR_DM_STG.AGPROD_PLAN_PRDR_SHR_FLTR_SUMM (PGM_YR, FARM_NBR, ST_FSA_CD, CNTY_FSA_CD, FSA_CROP_CD, FSA_CROP_TYPE_CD, INTN_USE_CD, PLNT_PRIM_STAT_CD, PLNT_SCND_STAT_CD, CERT_DT, CPLD_IND, CORE_CUST_ID, CROP_SHR_PCT, CROP_FLD_RPT_QTY, CROP_FLD_DTER_QTY)    
    (SELECT /*+ PARALLEL(16) */
        appbpf.pgm_yr,
        appd.farm_nbr,
        appd.ST_FSA_CD,
        ...
        INNER JOIN CAR_DM_STG.AG_PROD_PLAN_DIM appd
            ON     appbpf.AG_PROD_PLAN_DURB_ID = appd.AG_PROD_PLAN_DURB_ID
                ...
                AND (   appd.FSA_CROP_CD IN ('0011',
                                            ...
                                            '0714',
                                            '0904')
                    OR (    appd.FSA_CROP_CD = '0018'
                        AND (appd.FSA_CROP_TYPE_CD IN
                                ('LGR', 'MGR', 'SGR')))
                    OR (    appd.FSA_CROP_CD = '0047'
                        AND (appd.FSA_CROP_TYPE_CD IN
                                ('GAD', 'GAR', 'GAS'))))
