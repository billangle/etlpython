INSERT INTO edv.CROP_ACRG_RPT_BUS_PTY_LS (CROP_ACRG_RPT_BUS_PTY_L_ID, BUS_PTY_ID, LOAD_DT, DATA_EFF_STRT_DT, DATA_SRC_NM, DATA_STAT_CD, SRC_CRE_DT, SRC_LAST_CHG_DT, SRC_LAST_CHG_USER_NM, CROP_ACRG_RPT_ID, BUS_PTY_TYPE_CD, DATA_EFF_END_DT, LOAD_END_DT, HASH_DIF)
  (SELECT stg.*
   FROM
     (SELECT DISTINCT MD5 (COALESCE (BUS_PTY.PGM_YR::varchar(32), '-1') || '~~' || UPPER (COALESCE (TRIM (BUS_PTY.ST_FSA_CD) , '--')) || '~~' || UPPER (COALESCE (TRIM (BUS_PTY.CNTY_FSA_CD) , '--')) || '~~' || UPPER (COALESCE (TRIM (BUS_PTY.FARM_NBR) , '--')) || '~~' || COALESCE (BUS_PTY.CORE_CUST_ID::varchar(32), '-1')) AS CROP_ACRG_RPT_BUS_PTY_L_ID,
      ...
      ORDER BY BUS_PTY.CDC_DT) stg
   LEFT JOIN edv.CROP_ACRG_RPT_BUS_PTY_LS dv ON (stg.HASH_DIF = dv.HASH_DIF
