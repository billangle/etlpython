WITH stg AS
  (SELECT DISTINCT MD5 (UPPER (COALESCE (TRIM (SubQry.ST_FSA_CD) , '--')) || '~~' || UPPER (COALESCE (TRIM (SubQry.CNTY_FSA_CD) , '--')) || '~~' || UPPER (COALESCE (TRIM (SubQry.FARM_NBR) , '--')) || '~~' || COALESCE (TRIM (SubQry.TR_NBR::varchar(32)), '-1') || '~~' || UPPER (COALESCE (TRIM (SubQry.FLD_NBR) , '--')) || '~~' || UPPER (COALESCE (TRIM (CNTY_REF_MRT_RS.CNTY_NM) , '[NULL IN SOURCE]')) || '~~' || UPPER (COALESCE (TRIM (CNTY_REF_MRT_RS.CNTY_CAT_NM) , '[NULL IN SOURCE]')) || '~~' || UPPER (TRIM (COALESCE (CNTY_REF_MRT_RS.CNTY_NM, SubQry.CTRY_DIV_NM, '[NULL IN SOURCE]')))) AS FLD_CNTY_L_ID,
                   COALESCE (SubQry.FLD_ID,
                            ...
                            MD5 (TRIM (SubQry.ST_FSA_CD) || '~~' || TRIM (SubQry.CNTY_FSA_CD) || '~~' || TRIM (SubQry.FARM_NBR) || '~~' || TRIM (SubQry.TR_NBR::varchar(32)) || '~~' || TRIM (SubQry.FLD_NBR) || '~~' || TRIM (CNTY_REF_MRT_RS.CNTY_NM) || '~~' || TRIM (CNTY_REF_MRT_RS.CNTY_CAT_NM) || '~~' || TRIM (COALESCE (CNTY_REF_MRT_RS.CNTY_NM, SubQry.CTRY_DIV_NM, '[NULL IN SOURCE]')) || '~~' || SubQry.FLD_ID || '~~' || TRIM (SubQry.DATA_STAT_CD) || '~~' || TO_CHAR (SubQry.SRC_CRE_DT, 'YYYY-MM-DD HH24:MI:SS.FF') || '~~' || TRIM (SubQry.SRC_CRE_USER_NM) || '~~' || TO_CHAR (SubQry.SRC_LAST_CHG_DT, 'YYYY-MM-DD HH24:MI:SS.FF') || '~~' || TRIM (SubQry.SRC_LAST_CHG_USER_NM) || '~~' || TO_CHAR (SubQry.DATA_IACTV_DT, 'YYYY-MM-DD HH24:MI:SS.FF') || '~~' || TRIM (SubQry.ST_ANSI_CD) || '~~' || TRIM (SubQry.CNTY_ANSI_CD) || '~~' || SubQry.PGM_YR) AS HASH_DIF,
...
INSERT INTO edv.FLD_CNTY_LS (FLD_CNTY_L_ID, FLD_ID, LOAD_DT, LOAD_END_DT, DATA_EFF_END_DT, DATA_EFF_STRT_DT, DATA_SRC_NM, HASH_DIF, DATA_STAT_CD, SRC_CRE_DT, SRC_CRE_USER_NM, SRC_LAST_CHG_DT, SRC_LAST_CHG_USER_NM, DATA_IACTV_DT, ST_ANSI_CD, CNTY_ANSI_CD, PGM_YR)
SELECT stg.FLD_CNTY_L_ID,
       stg.FLD_ID,
       stg.LOAD_DT,
       stg.LOAD_DT,
       ...
       stg.PGM_YR
FROM stg
WHERE stg.FLD_CNTY_L_ID IN
    (SELECT DISTINCT FLD_CNTY_L_ID
     FROM EDV.FLD_CNTY_L)
  AND NOT EXISTS
    (SELECT '1'
     FROM remainder dv
