...
def handler(event, context):
    ...
    if no_files:
        subject =f"No Incremental Files available to Process for Conservation Payments Raw-DM on {today} in CERT"
        message = (
            f"No incremental files found for job {job_id} on {today}.\n\n"
            "Please check your source if you were expecting data."
        )
    else:
        subject = f"Conservation Payments Raw-DM Processed {len(files)} file(s) processed on {today} in CERT"
        file_list_str = "\n".join(files)
        message = (
            f"Conservation Payments RAW-DM Job {job_id} on {today} successfully processed {len(files)} file(s):\n\n"
            f"{file_list_str}"
        )
