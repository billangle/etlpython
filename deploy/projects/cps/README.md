...
- `CPS` is the project code used in the command examples below.
...
There are two deploy entry points involved for CPS:

- Master deployer: deploy/deploy.py
- CPS local deployer: deploy/projects/cps/deploy.py

Relationship:

- The master deployer is a router/orchestrator for all projects.
- When called with --project-type cps, it imports and calls projects.cps.deploy.deploy(cfg, region).
- The CPS local deployer performs all CPS-specific work (Glue, Lambda, Crawler, and Step Function deployment, including ASL token replacement).

Practical usage:

- Standard entrypoint (recommended):
	python deploy.py --config config/cps/steamdev.json --region us-east-1 --project-type cps
- Direct CPS module execution (for focused debugging):
	python projects/cps/deploy.py --config config/cps/steamdev.json --region us-east-1
...
- project: must resolve to CPS naming context
- artifacts.artifactBucket: S3 bucket for deployed Glue scripts
- artifacts.prefix: object prefix for deployment artifacts (for example cps/)
- stepFunctions.roleArn: IAM role ARN for Step Functions state machines
- secretId: secret used by CPS lambdas/Glue job arguments
...
- strparams.jobIdKeyParam: S3 object key for the job id file (for example cps/etl-jobs/job_id.json)
