...
def lambda_handler(event, context):
    ...
    if no_files:
        subject = f"No Incremental Files for CPS Raw-DM on {today}{env_suffix}"
        message = (
        ...
        subject = f"CPS Raw-DM: {len(display_files)} file(s) processed on {today}{env_suffix}"
        message = (
            f"CPS RAW-DM Job {job_id} on {today} successfully processed "
