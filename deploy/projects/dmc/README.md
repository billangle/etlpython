...
DMC pipeline deploy project for Glue, Lambda, and Step Functions resources.
...
- Branch naming: `<PROJECT>-<JIRA-TICKET>` (example: `DMC-POPSUP-7557`)
- `DMC` is the project code used in the command examples below.
...
git checkout -b DMC-POPSUP-7557

# Commit and push (triggers DEV automation)
git add .
git commit -m "DMC-POPSUP-7557: describe change"
git push -u origin DMC-POPSUP-7557

# Daily sync main into feature branch to reduce merge conflicts
git fetch origin
git checkout main
git pull origin main
git checkout DMC-POPSUP-7557
git merge main
git push
```
...
Examples:
- FSA-FPACDEV-DMC-LandingFiles
- FSA-FPACDEV-DMC-Raw-DM
- FSA-FPACDEV-DMC-MAIN

## Wiring Model

DMC Step Functions are wired using explicit __PLACEHOLDER__ tokens in ASL files.
...
There are two deploy entry points involved for DMC:

- Master deployer: deploy/deploy.py
- DMC local deployer: deploy/projects/dmc/deploy.py

Relationship:

- The master deployer is a router/orchestrator for all projects.
- When called with --project-type dmc, it imports and calls projects.dmc.deploy.deploy(cfg, region).
- The DMC local deployer performs all DMC-specific work (Glue, Lambda, Crawler, and Step Function deployment, including ASL token replacement).

Practical usage:

- Standard entrypoint (recommended):
	python deploy.py --config config/dmc/steamdev.json --region us-east-1 --project-type dmc
- Direct DMC module execution (for focused debugging):
	python projects/dmc/deploy.py --config config/dmc/steamdev.json --region us-east-1
...
- config/dmc/dev.json
- config/dmc/steamdev.json
- config/dmc/prod.json

The DMC deployer reads resource settings directly from these configs, especially:
...
- project: must resolve to DMC naming context
- artifacts.artifactBucket: S3 bucket for deployed Glue scripts
- artifacts.prefix: object prefix for deployment artifacts (for example dmc/)
- stepFunctions.roleArn: IAM role ARN for Step Functions state machines
- secretId: secret used by DMC lambdas/Glue job arguments
...
- strparams.jobIdKeyParam: S3 object key for the job id file (for example dmc/etl-jobs/job_id.json)
...
DMC Step Function ASL files under states/ now use explicit placeholder tokens that are replaced in deploy.py at deployment time.
...
Development:
python deploy.py --config config/dmc/dev.json --region us-east-1 --project-type dmc

Steamdev:
python deploy.py --config config/dmc/steamdev.json --region us-east-1 --project-type dmc

Production:
python deploy.py --config config/dmc/prod.json --region us-east-1 --project-type dmc

## Unit Tests

Run DMC deploy regression tests:

python -m unittest deploy.projects.dmc.tests.test_deploy -v