...
COALESCE(APE.data_eff_end_dt, TO_TIMESTAMP('9999-12-31','YYYY-MM-DD')) data_eff_end_dt,
APE.ARC_PLC_ELCT_CD ARC_PLC_ELCT_CD,
APE.DMN_VAL_NM ARC_PLC_ELCT_NM,
APE.DMN_VAL_DESC ARC_PLC_ELCT_DESC,
APE.dply_seq_nbr arc_plc_elct_srgt_id,
...
WHERE CAST(APE.DATA_EFF_STRT_DT as DATE) =  TO_TIMESTAMP('{V_CDC_DT}','YYYY-MM-DD') 
AND EXISTS ( SELECT '1' FROM EDV.ARC_PLC_ELCT_RS APE_IN
             WHERE APE_IN.ARC_PLC_ELCT_CD = APE.ARC_PLC_ELCT_CD
             AND CAST(APE_IN.DATA_EFF_END_DT as DATE) > TO_TIMESTAMP('{V_CDC_DT}','YYYY-MM-DD')
             ) 
AND APH.DURB_ID IS NOT NULL            
) DM WHERE DM.ROW_NUM_PART = 1
)
SELECT ud.arc_plc_elct_durb_id,
		ud.arc_plc_elct_srgt_id, 
	   1 as cur_rcd_ind, 
	   ud.data_eff_strt_dt,
	   data_eff_end_dt, 
