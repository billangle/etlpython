...
(
    ...
    WHERE       CAST(FRLS.DATA_EFF_END_DT AS DATE) =  TO_TIMESTAMP('{V_CDC_DT}','YYYY-MM-DD')
    ...
    WHERE       CAST(RR.DATA_EFF_END_DT AS DATE) = TO_TIMESTAMP('{V_CDC_DT}','YYYY-MM-DD')
),
TMP_MIDAS_FARM_RCON_LS AS
(
    SELECT      DISTINCT FARM_RCON_L_ID
    FROM        EDV.MIDAS_FARM_RCON_LS MFRLS
    WHERE       CAST(MFRLS.DATA_EFF_END_DT AS DATE) =  TO_TIMESTAMP('{V_CDC_DT}','YYYY-MM-DD')
    ...
    FROM        (
                    ...
                    WHERE       CAST(FYS.DATA_EFF_END_DT AS DATE) =  TO_TIMESTAMP('{V_CDC_DT}','YYYY-MM-DD')
...
LEFT JOIN CMN_DIM_DM_STG.FARM_DIM FD_RSLT ON (
 ...
 WHERE 
  (
 (
 CAST(FRLS.DATA_EFF_END_DT AS DATE) =  TO_TIMESTAMP('{V_CDC_DT}','YYYY-MM-DD')
 )
 OR
 (
...
COALESCE(CAST(MFRLS.TM_PRD_NM AS INTEGER), FIRST_VALUE(FYS.PGM_YR) OVER (PARTITION BY FS.FARM_H_ID ORDER BY FYS.PGM_YR ASC )) AS PGM_YR
 ...
 LEFT JOIN CMN_DIM_DM_STG.FARM_DIM FD_RSLT ON (FH_RSLT.DURB_ID = COALESCE(FD_RSLT.FARM_DURB_ID,-1)
  ...
  LEFT JOIN EDV.FARM_YR_HS FYS ON (FH.FARM_H_ID = COALESCE(FYS.FARM_H_ID,'baf6dd71fe45fe2f5c1c0e6724d514fd') 
                           AND CAST(FYS.DATA_EFF_END_DT AS DATE) = TO_TIMESTAMP('{V_CDC_DT}','YYYY-MM-DD')
                            ) 
...
LEFT JOIN CMN_DIM_DM_STG.FSA_ST_CNTY_DIM FSD_ADM ON (
 ...
 WHERE 
  (
 (
 CAST(MFRLS.DATA_EFF_END_DT AS DATE) =  TO_TIMESTAMP('{V_CDC_DT}','YYYY-MM-DD')
