...
DISTINCT
COALESCE(CAST(MFX.TM_PRD_NM AS INTEGER), FIRST_VALUE(FS.PGM_YR) OVER (PARTITION BY MFX.RSLT_ST_FSA_CD,MFX.RSLT_CNTY_FSA_CD ,MFX.RSLT_FARM_NBR ORDER BY FS.PGM_YR ASC )) AS PGM_YR, 
...
LAS_PRNT.DATA_EFF_STRT_DT DESC,LAS_RSLT.DATA_EFF_STRT_DT DESC ) AS  ROW_NUM_PART
 ...
 (
 (
 CAST(MFX.DATA_EFF_STRT_DT AS DATE) =  TO_TIMESTAMP('{ETL_DATE}','YYYY-MM-DD')
 AND CAST(MFX.DATA_EFF_END_DT AS DATE) > TO_TIMESTAMP('{ETL_DATE}','YYYY-MM-DD')
 )
 OR
 (
 ...
 OR
 (
 CAST(LAS_PRNT.DATA_EFF_STRT_DT AS DATE) =  TO_TIMESTAMP('{ETL_DATE}','YYYY-MM-DD')
 AND CAST(LAS_PRNT.DATA_EFF_END_DT AS DATE) = TO_TIMESTAMP('{ETL_DATE}','YYYY-MM-DD')
 )
 OR
 (
 CAST(LAS_RSLT.DATA_EFF_STRT_DT AS DATE) =  TO_TIMESTAMP('{ETL_DATE}','YYYY-MM-DD')
 AND CAST(LAS_RSLT.DATA_EFF_END_DT AS DATE) = TO_TIMESTAMP('{ETL_DATE}','YYYY-MM-DD')
 ) 
 )
 AND CAST(MFX.DATA_EFF_STRT_DT AS DATE) <=  TO_TIMESTAMP('{ETL_DATE}','YYYY-MM-DD')
 AND CAST(FS.DATA_EFF_STRT_DT AS DATE)  <=  TO_TIMESTAMP('{ETL_DATE}','YYYY-MM-DD')
 AND CAST(LAS_PRNT.DATA_EFF_STRT_DT AS DATE)  <=  TO_TIMESTAMP('{ETL_DATE}','YYYY-MM-DD')
 AND CAST(LAS_RSLT.DATA_EFF_STRT_DT AS DATE)  <=  TO_TIMESTAMP('{ETL_DATE}','YYYY-MM-DD') 
