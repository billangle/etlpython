...
SELECT DISTINCT FARM_YR_HS.FARM_H_ID INCR_DR_ID,  
                                   FARM_YR_HS.PGM_YR 
FROM EDV.FARM_YR_HS FARM_YR_HS, EDV.FARM_H FARM_H
WHERE  COALESCE(FARM_YR_HS.FARM_H_ID,'baf6dd71fe45fe2f5c1c0e6724d514fd') = FARM_H.FARM_H_ID 
AND CAST(DATA_EFF_STRT_DT AS DATE) =  TO_TIMESTAMP('{V_CDC_DT}','YYYY-MM-DD')  
AND CAST(DATA_EFF_END_DT AS DATE) >  TO_TIMESTAMP('{V_CDC_DT}','YYYY-MM-DD') 
UNION 
SELECT DISTINCT FARM_H.FARM_H_ID INCR_DR_ID,  
                                   FARM_YR_HS.PGM_YR 
FROM EDV.LOC_AREA_MRT_SRC_RS LOC_AREA_MRT_SRC_RS, EDV.FARM_H FARM_H, EDV.FARM_YR_HS FARM_YR_HS
...
SELECT DISTINCT  FARM_YR_DCP_HS.FARM_H_ID INCR_DR_ID,  
                                    FARM_YR_HS.PGM_YR 
FROM EDV.FARM_YR_DCP_HS FARM_YR_DCP_HS, EDV.FARM_YR_HS FARM_YR_HS
WHERE (CAST(FARM_YR_DCP_HS.DATA_EFF_STRT_DT AS DATE) =  TO_TIMESTAMP('{V_CDC_DT}','YYYY-MM-DD')  
      OR CAST(FARM_YR_DCP_HS.DATA_EFF_END_DT AS DATE) =   TO_TIMESTAMP('{V_CDC_DT}','YYYY-MM-DD'))   
...
SELECT DISTINCT  FARM_YR_HS.FARM_H_ID INCR_DR_ID,  
                                    FARM_YR_HS.PGM_YR 
FROM EDV.ARC_PLC_ELG_DTER_RS ARC_PLC_ELG_DTER_RS, EDV.FARM_YR_HS FARM_YR_HS
...
ON (APEDRH.ARC_PLC_ELG_DTER_CD = COALESCE(APEDRS.ARC_PLC_ELG_DTER_CD,'[NULL IN SOURCE]') 
    AND CAST(APEDRS.DATA_EFF_END_DT AS DATE) = TO_TIMESTAMP('9999-12-31', 'YYYY-MM-DD') 
...
WHERE COALESCE(CAST(FS_DATA_EFF_STRT_DT as DATE ),TO_TIMESTAMP('1111-12-31','YYYY-MM-DD')) <=  TO_TIMESTAMP('{V_CDC_DT}','YYYY-MM-DD') 
AND CAST(COALESCE(LAS_ADM_DATA_EFF_STRT_DT,TO_TIMESTAMP('1111-12-31','YYYY-MM-DD')) AS DATE) <=  TO_TIMESTAMP('{V_CDC_DT}','YYYY-MM-DD') 
AND CAST(COALESCE(FDS_DATA_EFF_STRT_DT,TO_TIMESTAMP('1111-12-31','YYYY-MM-DD')) AS DATE) <=  TO_TIMESTAMP('{V_CDC_DT}','YYYY-MM-DD') 
