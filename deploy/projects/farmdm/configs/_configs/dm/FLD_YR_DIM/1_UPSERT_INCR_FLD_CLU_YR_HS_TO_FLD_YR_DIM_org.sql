...
And FLD_H_ID in
   (
    ...
    UNION
     ...
     Select DMN_VAL_ID FROM EDV.FLD_HEL_STAT_RS  WHERE CAST(DATA_EFF_STRT_DT as DATE) = TO_TIMESTAMP('{ETL_DATE}','YYYY-MM-DD') )
    UNION
     SELECT FLD_H_ID   from EDV.FLD_CLU_YR_HS where LAND_CLS_ID in (
     Select DMN_VAL_ID  FROM EDV.LAND_CLS_RS  WHERE CAST(DATA_EFF_STRT_DT as DATE) = TO_TIMESTAMP('{ETL_DATE}','YYYY-MM-DD') )
    )
 )
Select 
dv.FLD_DURB_ID ,
...
('FLD_DURB_ID:'||DV.FLD_DURB_ID ||'PGM_YR:'||dv.PGM_YR ) BUS_KEY_ERR 
 ...
 where   TO_TIMESTAMP('{ETL_DATE}','YYYY-MM-DD') >= CAST(FLD_CLU_YR_HS.DATA_EFF_STRT_DT AS DATE)
  AND TO_TIMESTAMP('{ETL_DATE}','YYYY-MM-DD') < CAST(FLD_CLU_YR_HS.DATA_EFF_END_DT AS DATE) 
 ...
 ON (  COALESCE(DV_DR.HEL_STAT_ID,-1) = COALESCE(HS.DMN_VAL_ID ,-1)
  AND TO_TIMESTAMP('{ETL_DATE}','YYYY-MM-DD') >= CAST(HS.DATA_EFF_STRT_DT AS DATE)
  AND TO_TIMESTAMP('{ETL_DATE}','YYYY-MM-DD') < CAST(HS.DATA_EFF_END_DT AS DATE) 
 )
 Left join   EDV.LAND_CLS_RS LNDS
 ON (  COALESCE(DV_DR.LAND_CLS_ID,-1) = COALESCE(LNDS.DMN_VAL_ID ,-1)
  AND TO_TIMESTAMP('{ETL_DATE}','YYYY-MM-DD') >= CAST(LNDS.DATA_EFF_STRT_DT AS DATE)
  AND TO_TIMESTAMP('{ETL_DATE}','YYYY-MM-DD') < CAST(LNDS.DATA_EFF_END_DT AS DATE) 
 )
 Left JOIN  
 (   select CONG_DIST_CD ,ST_FSA_CD , CONG_DIST_NM  ,ROW_NUMBER() OVER ( PARTITION BY CONG_DIST_CD ,ST_FSA_CD ORDER BY CRE_DT DESC
         ...
         where  TO_TIMESTAMP('{ETL_DATE}','YYYY-MM-DD') >= CAST(CONG_DIST_DIM.DATA_EFF_STRT_DT AS DATE)
        AND TO_TIMESTAMP('{ETL_DATE}','YYYY-MM-DD') < CAST(CONG_DIST_DIM.DATA_EFF_END_DT AS DATE)    
 ) CONG_DIST_DIM
 ...
 Left JOIN cmn_dim_dm_stg.ANSI_ST_CNTY_DIM 
   ON ( COALESCE(upper(trim(DV_DR.ST_ANSI_CD)),'--') =   COALESCE(upper(trim(ANSI_ST_CNTY_DIM.ST_ANSI_CD)),'--')
        ...
        AND TO_TIMESTAMP('{ETL_DATE}','YYYY-MM-DD') >= CAST(ANSI_ST_CNTY_DIM.DATA_EFF_STRT_DT AS DATE)
        AND TO_TIMESTAMP('{ETL_DATE}','YYYY-MM-DD') < CAST(ANSI_ST_CNTY_DIM.DATA_EFF_END_DT AS DATE)         )
 LEFT  JOIN 
    (  SELECT FSA_ST_CNTY_SRGT_ID,FSA_ST_CNTY_DURB_ID ,ST_FSA_CD ,CNTY_FSA_CD,ST_NM,CNTY_NM ,ST_ABR
         ...
         WHERE  TO_TIMESTAMP('{ETL_DATE}','YYYY-MM-DD') >= CAST(FSA_ST_CNTY_DIM.DATA_EFF_STRT_DT AS DATE)
        AND TO_TIMESTAMP('{ETL_DATE}','YYYY-MM-DD') < CAST(FSA_ST_CNTY_DIM.DATA_EFF_END_DT AS DATE)
    ) FSA_ST_CNTY_DIM 
