with incr_RMA_TRP_CW_EXCP_DIM  as
        (   SELECT RMA_CW_EXCP_CD dr_id
            FROM EDV.RMA_CW_EXCP_RH
...
, upsert_data as ( 
select 
dv_dr.durb_id AS RMA_TR_PRDR_CW_EXCP_DURB_ID
...
where data_src_nm <> 'SYSGEN' AND RMA_CW_EXCP_CD  in (select dr_id from incr_RMA_TRP_CW_EXCP_DIM )) dv_dr
LEFT JOIN 
(
select 
RMA_CW_EXCP_CD,
...
select 	rma_tr_prdr_cw_excp_durb_id,
		...
		rma_cw_excp_desc
from upsert_data 
where rma_tr_prdr_cw_excp_durb_id > 0
order by rma_tr_prdr_cw_excp_durb_id asc