...
SELECT DISTINCT TRLS.PGM_YR PGM_YR
 ...
 WHERE 
  (
 (
 cast(TRLS.DATA_EFF_END_DT as date) =  TO_TIMESTAMP('{V_CDC_DT}','YYYY-MM-DD')
 )
 OR
 (
 cast(RR.DATA_EFF_END_DT as date) =  TO_TIMESTAMP('{V_CDC_DT}','YYYY-MM-DD')
 )
 OR
 (
 cast(LAS_ADM.DATA_EFF_END_DT as date) =  TO_TIMESTAMP('{V_CDC_DT}','YYYY-MM-DD')
 )
 OR
 (
 cast(FCS.DATA_EFF_END_DT as date) =  TO_TIMESTAMP('{V_CDC_DT}','YYYY-MM-DD')
 )
 )
...
LAS_ADM.DATA_EFF_STRT_DT DESC ) AS  ROW_NUM_PART
    ...
    FROM (
        ...
        FROM  (
            ...
            INNER JOIN (
                ...
                WHERE DATA_EFF_END_DT = TO_TIMESTAMP('{V_CDC_DT}', 'YYYY-MM-DD')
 ...
 OR
 (
 cast(FTLS.DATA_EFF_END_DT as date) =  TO_TIMESTAMP('{V_CDC_DT}','YYYY-MM-DD')
 )
 OR
 (
 cast(LAS_ADM.DATA_EFF_END_DT as date) =  TO_TIMESTAMP('{V_CDC_DT}','YYYY-MM-DD')
