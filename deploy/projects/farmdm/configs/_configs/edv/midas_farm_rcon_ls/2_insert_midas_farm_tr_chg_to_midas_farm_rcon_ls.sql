...
ON (MIDAS_FARM_TR_CHG.FARM_CHG_TYPE_ID = FARM_CHG_TYPE_RS.DMN_VAL_ID) where MIDAS_FARM_TR_CHG.FARM_CHG_TYPE_ID = 1142 and MIDAS_FARM_TR_CHG.CDC_OPER_CD IN ('I','UN')
order by MIDAS_FARM_TR_CHG.CDC_DT
) stg
LEFT JOIN  EDV.MIDAS_FARM_RCON_LS dv
ON (
stg.HASH_DIF = dv.HASH_DIF
and dv.LOAD_END_DT = TO_TIMESTAMP('9999-12-31','YYYY-MM-DD')
)
where dv.HASH_DIF is null
and not exists (select 1 from EDV.MIDAS_FARM_RCON_LS ls
