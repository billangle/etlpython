INSERT into
EDV.MIDAS_FARM_XFR_LS(FARM_RCON_L_ID,
...
where MIDAS_FARM_TR_CHG.FARM_CHG_TYPE_ID = 1141 and MIDAS_FARM_TR_CHG.CDC_OPER_CD IN ('I','UN')
And MD5(MD5(upper(coalesce(MIDAS_FARM_TR_CHG.PRNT_ST_FSA_CD,'--') ||'~~'|| coalesce(MIDAS_FARM_TR_CHG.PRNT_CNTY_FSA_CD,'--') ||'~~'|| coalesce(MIDAS_FARM_TR_CHG.PRNT_FARM_NBR,'--')))||'~~'||MD5(upper(coalesce(MIDAS_FARM_TR_CHG.RSLT_ST_FSA_CD,'--') ||'~~'|| coalesce(MIDAS_FARM_TR_CHG.RSLT_CNTY_FSA_CD,'--') ||'~~'|| coalesce(MIDAS_FARM_TR_CHG.RSLT_FARM_NBR,'--'))))
IN (SELECT FARM_RCON_L_ID FROM Edv.FARM_RCON_L)

UNION

  SELECT DISTINCT 
 MD5('-1') FARM_RCON_L_ID,
MIDAS_FARM_TR_CHG.MIDAS_FARM_TR_CHG_ID MIDAS_FARM_TR_CHG_ID,
MIDAS_FARM_TR_CHG.LOAD_DT LOAD_DT,
MIDAS_FARM_TR_CHG.CDC_DT DATA_EFF_STRT_DT,
MIDAS_FARM_TR_CHG.DATA_SRC_NM DATA_SRC_NM,
...
where MIDAS_FARM_TR_CHG.FARM_CHG_TYPE_ID = 1141 and MIDAS_FARM_TR_CHG.CDC_OPER_CD IN ('I','UN')
And MD5(MD5(upper(coalesce(MIDAS_FARM_TR_CHG.PRNT_ST_FSA_CD,'--') ||'~~'|| coalesce(MIDAS_FARM_TR_CHG.PRNT_CNTY_FSA_CD,'--') ||'~~'|| coalesce(MIDAS_FARM_TR_CHG.PRNT_FARM_NBR,'--')))||'~~'||MD5(upper(coalesce(MIDAS_FARM_TR_CHG.RSLT_ST_FSA_CD,'--') ||'~~'|| coalesce(MIDAS_FARM_TR_CHG.RSLT_CNTY_FSA_CD,'--') ||'~~'|| coalesce(MIDAS_FARM_TR_CHG.RSLT_FARM_NBR,'--'))))
Not IN (SELECT FARM_RCON_L_ID FROM Edv.FARM_RCON_L)
) stg
 LEFT JOIN  EDV.MIDAS_FARM_XFR_LS dv
 ON (
stg.HASH_DIF = dv.HASH_DIF
and dv.LOAD_END_DT = TO_TIMESTAMP('9999-12-31','YYYY-MM-DD')
)
where dv.HASH_DIF is null
and not exists (select 1 from EDV.MIDAS_FARM_XFR_LS ls
