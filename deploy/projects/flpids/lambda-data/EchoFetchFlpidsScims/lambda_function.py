...
def lambda_handler(event, context):
    ...
    {
        "file_pattern": "(MOYWRSC_[A-Z]{4})\\.txt$",        # required
        "echo_folder": "plas",                              # required
        "pipeline": "FLPIDS_RPT (SCIMS)",                   # required
        "step": "FLPIDS_RPT (SCIMS) Landing Zone",          # required
        "targets": ["direct_attribution_confirmation"],     # optional, default: []
        "to_queue": false,                                  # optional, default: false
        "header": false                                     # optional, default: true
    }
    """
    print("Input event:\n", event)
    env = os.environ["env"]
    ...
    def echo_file_transfer(file):
        ...
        with iFTP_TLS(timeout=600) as ftps:
            ...
            with DARTSession(
                ...
                to_queue=to_queue,
                override_prev_run= True #if datetime.now().strftime("%Y%m%d") == "20250616" else False
            ) as session:
