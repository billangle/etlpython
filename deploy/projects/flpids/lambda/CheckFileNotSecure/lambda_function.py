...
def _build_https_ssl_context(tls_mode: str, debug: bool = False) -> ssl.SSLContext:
    """
    tls_mode:
      - "strict": normal TLS verification + hostname verification (default)
      - "no_hostname": verify CA chain, but skip hostname/IP matching
      - "insecure": no verification (accepts self-signed, etc.)  **least secure**
    """
    mode = (tls_mode or "strict").strip().lower()
    if mode == "strict":
        if debug:
            print("[DEBUG] Jenkins TLS: strict (verify cert + hostname)")
        return ssl.create_default_context()

    if mode == "no_hostname":
        if debug:
            print("[DEBUG] Jenkins TLS: no_hostname (verify cert chain, skip hostname/IP check)")
        ctx = ssl.create_default_context()
        ctx.check_hostname = False
        # verify_mode remains CERT_REQUIRED (default)
        return ctx

    if mode == "insecure":
        if debug:
            print("[DEBUG] Jenkins TLS: insecure (NO cert validation; NO hostname check)")
        ctx = ssl.create_default_context()
        ctx.check_hostname = False
        ctx.verify_mode = ssl.CERT_NONE
        return ctx

    raise ValueError(f"Unsupported jenkins_tls_mode={tls_mode}. Use strict|no_hostname|insecure")
...
def http_post_json(
    ...
    tls_mode: str = "strict",  # strict|no_hostname|insecure
) -> dict:
    """
    POST JSON payload to url with optional HTTP Basic Auth and controllable TLS behavior.
    ...
    if debug:
        print(
            ...
            f"basic_auth={'enabled' if basic_auth else 'disabled'}, tls_mode={tls_mode}"
        )
    ...
    if url.lower().startswith("https://"):
        context = _build_https_ssl_context(tls_mode=tls_mode, debug=debug)
...
def lambda_handler(event, context):
    ...
    Optional:
      - debug: bool
      - verify_tls: bool                     (for FTPS curl only)
      - echo_subfolder: str
      - targets: list[str]
      ...
      - jenkins_tls_mode: str (default "strict") -> "strict" | "no_hostname" | "insecure"
          * Use "no_hostname" to avoid cert errors when calling https://<IP>/... (hostname mismatch)
          * Use "insecure" only if you must (disables all TLS verification)
    """
    ...
    try:
        ...
        verify_tls = bool(event.get("verify_tls", False))  # FTPS curl TLS verification

        # Jenkins TLS mode (strict|no_hostname|insecure) for HTTPS calls
        jenkins_tls_mode = (event.get("jenkins_tls_mode") or "strict").strip().lower()
        ...
        dbg(f"Jenkins HTTPS TLS mode: {jenkins_tls_mode}")
        ...
        result["jenkins_call"] = {
            ...
            "jenkins_tls_mode": jenkins_tls_mode,
        }
