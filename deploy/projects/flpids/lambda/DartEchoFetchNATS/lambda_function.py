...
def lambda_handler(event, context):
    ...
    {
        "file_pattern": "(FILE_NATS_\\w+)\\.(\\d{8})\\.csv$",           # required
        "echo_folder": "nats",                                          # required
        "pipeline": "FLPIDS_ODS",                                       # required
        "step": "FLPIDS_ODS Landing Zone",                              # required
    ...
    meta_client = DARTDatabase.client(
        service="lambda",
        glue_connection=f"FSA-CERT-PG-MDART", 
