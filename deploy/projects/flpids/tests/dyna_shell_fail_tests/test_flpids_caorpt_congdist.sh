...
# ----------------------------
# FAILURE TEST - invokes DynaCheckFile with TestFailureJob as the transfer Lambda.
# The transfer step will intentionally raise an exception, routing the state machine
# through the Catch branch (FinalizeJobOnCatch) to verify the FAILURE/ERROR path.
#
# Jenkins Job: FSA-PROD-DART-ECHO-FETCH-FLPIDS-CAORPT-CONGDIST
#
...
: "${FTPS_PORT:=21}"
: "${LAMBDA_ARN:=arn:aws:lambda:us-east-1:335965711887:function:FSA-steam-dev-FpacFLPIDS-TestFailureJob}"

: "${DDB_TABLE_NAME:=FSA-FileChecks}"
: "${DDB_PROJECT_GSI_NAME:=project_gsi_name}"
: "${SECRET_ID:=FSA-CERT-Secrets}"

JENKINS_JOB_NAME="FSA-PROD-DART-ECHO-FETCH-FLPIDS-CAORPT-CONGDIST"
PROJECT_NAME="${JENKINS_JOB_NAME#*ECHO-FETCH-}"
: "${STEP:=CAO_RPT Landing Zone (Weekly)}"
: "${ENV:=CERT}"
export PROJECT_NAME

# Defaults (regex can contain braces, so avoid shell parameter default syntax with embedded braces)
if [ -z "${FILE_PATTERN:-}" ]; then FILE_PATTERN='(\w{1,128})\.(\d{8})\.csv$'; fi
if [ -z "${ECHO_FOLDER:-}" ]; then ECHO_FOLDER='mrtxdb'; fi
if [ -z "${ECHO_SUBFOLDER:-}" ]; then ECHO_SUBFOLDER='weekly'; fi
if [ -z "${PIPELINE:-}" ]; then PIPELINE='CAO_RPT'; fi
...
RESP_FILE="${RESP_FILE:-response-fail.json}"
...
echo "  [FAILURE TEST]   LAMBDA_ARN points to TestFailureJob"
