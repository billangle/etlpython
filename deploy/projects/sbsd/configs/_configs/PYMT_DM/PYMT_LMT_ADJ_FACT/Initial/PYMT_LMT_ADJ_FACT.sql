...
INSERT INTO PYMT_DM_STG.PYMT_LMT_ADJ_FACT (CRE_DT,
                                           LAST_CHG_DT,
                                           DATA_EFF_STRT_DT,
                                           DATA_EFF_END_DT,
                                           CUST_SRGT_ID,
                                           CUST_DURB_ID,
                                           PYMT_LMT_ADJ_TYPE_SRGT_ID,
                                           ...
                                           FSA_ST_CNTY_SRGT_ID,
                                           SRC_LAST_CHG_DT,
                                           PYMT_LMT_ADJ_NOTE_TXT,
                                           PYMT_LMT_ADJ_AMT,
                                           DATA_STAT_CD,
                                           SRC_LAST_CHG_USER_NM)
SELECT   DISTINCT  
CURRENT_TIMESTAMP cre_dt, /*(BATCH_LD_DT in datastage*/
CURRENT_TIMESTAMP last_chg_dt,
 DM.DATA_EFF_STRT_DT,
...
FROM        (
                SELECT              /*+ PARALLEL(2) */ DISTINCT PRDR_PGM_PYMT_LMT_ADJ_LS.DATA_EFF_STRT_DT,
                                    COALESCE(CUST_DIM.CUST_SRGT_ID, -3) AS CUST_SRGT_ID,
                                    COALESCE(CUST_DIM.CUST_DURB_ID, -3) AS CUST_DURB_ID,
                                    COALESCE(PYMT_LMT_ADJ_TYPE_DIM.PYMT_LMT_ADJ_TYPE_SRGT_ID, -3) AS PYMT_LMT_ADJ_TYPE_SRGT_ID,
                                    COALESCE(PYMT_LMT_ADJ_TYPE_DIM.PYMT_LMT_ADJ_TYPE_DURB_ID, -3) AS PYMT_LMT_ADJ_TYPE_DURB_ID,
                ...
                FROM                 EDV.PRDR_PGM_PYMT_LMT_ADJ_LS
                                    ...
                                    LEFT OUTER JOIN EDV.CORE_CUST_RPT_LOC_AREA_L ON PRDR_PGM_PYMT_LMT_ADJ_L.CORE_CUST_ID = COALESCE(CORE_CUST_RPT_LOC_AREA_L.CORE_CUST_ID, -1)
                                    ...
                                    LEFT OUTER JOIN EDV.SBSD_LOC_AREA_RS ON COALESCE(CORE_CUST_RPT_LOC_AREA_LS.CNTY_FSA_SVC_CTR_ID, 0) = SBSD_LOC_AREA_RS.CNTY_FSA_SVC_CTR_ID
                                    LEFT OUTER JOIN CMN_DIM_DM_STG.FSA_ST_CNTY_DIM  ON ( COALESCE(SBSD_LOC_AREA_RS.ST_CNTY_FSA_CD, 'X') = CONCAT(FSA_ST_CNTY_DIM.ST_FSA_CD, FSA_ST_CNTY_DIM.CNTY_FSA_CD) AND FSA_ST_CNTY_DIM.CUR_RCD_IND = 1 )
                                    ...
                                    LEFT OUTER JOIN CMN_DIM_DM_STG.DT_DIM ON TO_CHAR(PRDR_PGM_PYMT_LMT_ADJ_LS.SRC_LAST_CHG_DT, 'YYYYMMDD')::INTEGER = DT_DIM.DT_ID
                WHERE               PRDR_PGM_PYMT_LMT_ADJ_LS.DATA_EFF_END_DT = TO_TIMESTAMP('9999-12-31','YYYY-MM-DD')
                       -- AND         TRUNC(PRDR_PGM_PYMT_LMT_ADJ_LS.DATA_EFF_STRT_DT) <= TO_TIMESTAMP('{ETL_START_DATE}','YYYY-MM-DD')
			AND CAST(PRDR_PGM_PYMT_LMT_ADJ_LS.DATA_EFF_STRT_DT AS date) <= TO_TIMESTAMP('{ETL_START_DATE}','YYYY-MM-DD')
