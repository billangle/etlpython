UPDATE EDV.ACCT_REF_TYPE_RS /*Replace with DV table Name*/
SET DATA_EFF_END_DT = TO_TIMESTAMP ('{ETL_START_TIMESTAMP}',
  ...
  (SELECT coalesce(max(load_dt), current_timestamp)
   FROM SBSD_STG.RCD_REF_ID_TYPE /*Replace with Staging table name*/
   WHERE CDC_DT = TO_TIMESTAMP ('{ETL_START_TIMESTAMP}',
                                'YYYY-MM-DD HH24:MI:SS.FF')/*Use CDC Start date variable*/ )
WHERE (coalesce(ACCT_REF_TYPE_CD, '0'))/*Use dv upd key(s)*/ NOT IN /*Identify Records that we have received in Staging for a given CDC date*/
    (SELECT coalesce(RCD_REF_ID_TYPE_CD, '0') /*Use DV upd key(s)*/ /*THIS SHOULD BE STAGING SRC PK*/
    ...
    (SELECT count(*)
     FROM
       (SELECT DV_CNT,
        ...
        FROM
          (SELECT count(DISTINCT ACCT_REF_TYPE_CD) DV_CNT /*Replace with DV src pk field name*/
           FROM EDV.ACCT_REF_TYPE_RS /*Replace with DV table name*/
           WHERE DATA_EFF_END_DT = TO_TIMESTAMP('9999-12-31', 'YYYY-MM-DD') ) dv,

          (SELECT count(DISTINCT ACCT_REF_TYPE_CD) UPD_CNT /*Replace with DV src pk field name*/
           FROM EDV.ACCT_REF_TYPE_RS /*Replace with DV table name*/
           WHERE (coalesce(ACCT_REF_TYPE_CD, '0'))/*Replace with DV src pk field name*/ NOT IN
               (SELECT coalesce(RCD_REF_ID_TYPE_CD, '0') /*Replace with Stage SRC pk Field name*/
                FROM SBSD_STG.RCD_REF_ID_TYPE /*Replace with Full load Stage Table Name*/
                WHERE cdc_dt = TO_TIMESTAMP ('{ETL_START_TIMESTAMP}',
