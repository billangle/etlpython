INSERT INTO EDV.CMB_PRDR_ACCT_MBR_LS (CMB_PRDR_ACCT_MBR_L_ID, LOAD_DT, DATA_EFF_STRT_DT, DATA_SRC_NM, DATA_STAT_CD, SRC_LAST_CHG_DT, SRC_LAST_CHG_USER_NM, CMB_ACCT_JOIN_DT, CMB_ACCT_LV_DT, CMB_PRDR_ACCT_MBR_ID, CMB_PRDR_ACCT_ID, SBSD_CUST_ID, DATA_EFF_END_DT, LOAD_END_DT)
  (SELECT stg.*
   FROM
     (SELECT DISTINCT MD5 (MD5 (coalesce(CMB_PRDR_ACCT_H.CMB_PRDR_ACCT_ID::varchar, '-1'))||coalesce(SBSD_CUST.CORE_CUST_ID, '-1')) AS CMB_PRDR_ACCT_MBR_L_ID,
      ...
      LEFT OUTER JOIN EDV.CMB_PRDR_ACCT_H ON (CMB_PRDR_ACCT_H.CMB_PRDR_ACCT_ID=CMB_ACCT_MBR.CMB_PRDR_ACCT_ID)
   ...
   LEFT JOIN EDV.CMB_PRDR_ACCT_MBR_LS dv ON (dv.CMB_PRDR_ACCT_MBR_L_ID= stg.CMB_PRDR_ACCT_MBR_L_ID
                                             AND coalesce(stg.DATA_STAT_CD, 'X') = coalesce(dv.DATA_STAT_CD, 'X')
                                             AND coalesce(stg.CMB_ACCT_JOIN_DT, current_date) = coalesce(dv.CMB_ACCT_JOIN_DT, current_date)
                                             AND coalesce(stg.CMB_ACCT_LV_DT, current_date) = coalesce(dv.CMB_ACCT_LV_DT, current_date)
                                             AND coalesce(stg.CMB_PRDR_ACCT_MBR_ID, 0) = coalesce(dv.CMB_PRDR_ACCT_MBR_ID, 0)
   ...
   WHERE dv.CMB_PRDR_ACCT_MBR_L_ID IS NULL )