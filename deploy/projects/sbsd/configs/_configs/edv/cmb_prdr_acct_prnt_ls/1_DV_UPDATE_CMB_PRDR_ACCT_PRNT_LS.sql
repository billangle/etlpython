...
ON (coalesce(dv.CMB_PRDR_ACCT_ID, 0)=coalesce(stg.CMB_PRDR_ACCT_ID, 0)) 
WHEN MATCHED 
  AND dv.LOAD_DT <> stg.LOAD_DT
  AND dv.LOAD_END_DT = to_date('9999-12-31', 'YYYY-MM-DD')
  AND (coalesce(stg.CMB_PRDR_ACCT_PRNT_L_ID, 'X') <> coalesce(dv.CMB_PRDR_ACCT_PRNT_L_ID, 'X')
       OR coalesce(stg.PRNT_CMB_PRDR_ACCT_ID, 0) <> coalesce(dv.PRNT_CMB_PRDR_ACCT_ID, 0)
