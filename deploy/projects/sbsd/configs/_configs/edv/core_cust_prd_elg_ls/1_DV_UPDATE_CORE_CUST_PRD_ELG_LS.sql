MERGE INTO EDV.CORE_CUST_PRD_ELG_LS dv 
USING (
...
WHEN MATCHED 
  ...
  AND (coalesce(stg.CORE_CUST_PRD_ELG_L_ID, 'X') <> coalesce(dv.CORE_CUST_PRD_ELG_L_ID, 'X')
       OR coalesce(stg.DATA_STAT_CD, 'X') <> coalesce(dv.DATA_STAT_CD, 'X')
       OR coalesce(stg.BUS_TYPE_CD, 'X') <> coalesce(dv.BUS_TYPE_CD, 'X')
       OR coalesce(stg.BUS_TYPE_EFF_DT, current_timestamp) <> coalesce(dv.BUS_TYPE_EFF_DT, current_timestamp)
       OR coalesce(stg.DFLT_ELG_IND, 0) <> coalesce(dv.DFLT_ELG_IND, 0)
       OR coalesce(stg.ELG_PRD_ID, 0) <> coalesce(dv.ELG_PRD_ID, 0)
       OR coalesce(stg.SBSD_PRD_CUST_ELG_PRFL_ID, 0) <> coalesce(dv.SBSD_PRD_CUST_ELG_PRFL_ID, 0))
THEN
UPDATE
