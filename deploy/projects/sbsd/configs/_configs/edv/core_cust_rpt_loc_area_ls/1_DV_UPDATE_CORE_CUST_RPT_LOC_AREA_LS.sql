...
WHEN MATCHED 
  ...
  AND (coalesce(stg.CORE_CUST_RPT_LOC_AREA_L_ID, 'X') <> coalesce(dv.CORE_CUST_RPT_LOC_AREA_L_ID, 'X')
       ...
       OR coalesce(stg.LOC_AREA_REF_ID, 0) <> coalesce(dv.LOC_AREA_REF_ID, 0)
       OR coalesce(stg.LOC_AREA_ID, 0) <> coalesce(dv.LOC_AREA_ID, 0)
       OR coalesce(stg.LOC_AREA_CAT_ID, 0) <> coalesce(dv.LOC_AREA_CAT_ID, 0)
       OR coalesce(stg.CTRY_DIV_ID, 0) <> coalesce(dv.CTRY_DIV_ID, 0)
       OR coalesce(stg.CTRY_DIV_REF_ID, 0) <> coalesce(dv.CTRY_DIV_REF_ID, 0))
THEN
UPDATE
