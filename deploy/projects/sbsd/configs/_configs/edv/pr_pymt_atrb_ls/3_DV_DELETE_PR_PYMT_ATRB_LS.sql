MERGE INTO EDV.PR_PYMT_ATRB_LS dv USING
  (SELECT DISTINCT MD5 (upper(COALESCE (trim(PYMT_ATRB_AUD.ST_FSA_CD), '--')) ||'~~'||upper(COALESCE (trim(PYMT_ATRB_AUD.CNTY_FSA_CD), '--')) ||'~~'||upper(COALESCE (trim(PYMT_ATRB_AUD.PRPS_PYMT_ACCT_PGM_CD), '--')) ||'~~'||COALESCE (PYMT_ATRB_AUD.PAID_CORE_CUST_ID, '-1')) AS DIR_ATRB_PRPS_PYMT_L_ID,
   ...
   WHERE PYMT_ATRB_AUD.PRNT_PYMT_ATRB_ID IS NULL
     AND PYMT_ATRB_AUD.CDC_OPER_CD='D'
     AND date(PYMT_ATRB_AUD.CDC_DT) = date(TO_TIMESTAMP ('{ETL_DATE}', 'YYYY-MM-DD HH24:MI:SS.FF'))
   ORDER BY PYMT_ATRB_AUD.CDC_DT) stg ON (coalesce(stg.PRPS_PYMT_ID, 0) = coalesce(dv.PRPS_PYMT_ID, 0)
                                          AND coalesce(stg.DATA_ACTN_DT, current_timestamp) = coalesce(dv.DATA_ACTN_DT, current_timestamp)
                                          AND coalesce(stg.PYMT_ATRB_ID, 0) = coalesce(dv.PYMT_ATRB_ID, 0)) 
WHEN MATCHED
