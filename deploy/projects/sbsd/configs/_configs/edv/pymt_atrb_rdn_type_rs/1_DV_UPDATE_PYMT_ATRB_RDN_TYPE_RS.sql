MERGE INTO EDV.PYMT_ATRB_RDN_TYPE_RS dv USING
  (SELECT DISTINCT COALESCE (trim(PYMT_ATRB_RDN_TYPE.PYMT_ATRB_RDN_TYPE_CD),
                       ...
                       MD5 (trim(PYMT_ATRB_RDN_TYPE.PYMT_ATRB_RDN_TYPE_CD)||'~~'||to_char(PYMT_ATRB_RDN_TYPE.CRE_DT, 'YYYY-MM-DD HH24:MI:SS.FF')||'~~'||trim(PYMT_ATRB_RDN_TYPE.CRE_USER_NM)||'~~'||to_char(PYMT_ATRB_RDN_TYPE.LAST_CHG_DT, 'YYYY-MM-DD HH24:MI:SS.FF')||'~~'||trim(PYMT_ATRB_RDN_TYPE.LAST_CHG_USER_NM)||'~~'||trim(PYMT_ATRB_RDN_TYPE.PYMT_ATRB_RDN_CAT_CD)||'~~'||trim(PYMT_ATRB_RDN_TYPE.PYMT_ATRB_RDN_TYPE_NM)||'~~'||trim(PYMT_ATRB_RDN_TYPE.PCT_BASE_RDN_IND)) HASH_DIF
   ...
   ORDER BY PYMT_ATRB_RDN_TYPE.CDC_DT) stg ON (coalesce(stg.PYMT_ATRB_RDN_TYPE_CD, 'DFLT') = COALESCE (dv.PYMT_ATRB_RDN_TYPE_CD,
                                                                                             'DFLT')) 
WHEN MATCHED
