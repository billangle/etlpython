MERGE INTO EDV.PYMT_ATRB_SIM_MBR_HLS dv USING
  (SELECT DISTINCT MD5 (upper(COALESCE (trim(PYMT_ATRB_SIM.ST_FSA_CD), '')) ||'~~'||upper(COALESCE (trim(PYMT_ATRB_SIM.CNTY_FSA_CD), '')) ||'~~'||upper(COALESCE (trim(PYMT_ATRB_SIM.PRPS_PYMT_SIM_ACCT_PGM_CD), '')) ||'~~'||PYMT_ATRB_SIM.MBR_CORE_CUST_ID ||'~~'||PYMT_ATRB_SIM.PRNT_PYMT_ATRB_CORE_CUST_ID ||'~~'||PYMT_ATRB_SIM.PAID_CORE_CUST_ID) AS PYMT_ATRB_MBR_HL_ID,
   ...
   WHERE PYMT_ATRB_SIM.PRNT_PYMT_ATRB_SIM_ID IS NOT NULL
     AND PYMT_ATRB_SIM.CDC_OPER_CD='D'
     AND date(PYMT_ATRB_SIM.CDC_DT) = date(TO_TIMESTAMP ('{ETL_START_TIMESTAMP}', 'YYYY-MM-DD HH24:MI:SS.FF'))
   ORDER BY PYMT_ATRB_SIM.CDC_DT) stg ON (coalesce(stg.PYMT_ATRB_SIM_ID, 0) = coalesce(dv.PYMT_ATRB_SIM_ID, 0)) 
WHEN MATCHED
