...
WHEN MATCHED
  ...
  AND (coalesce(stg.PYMT_PGM_NM, 'X') <> coalesce(dv.PYMT_PGM_NM, 'X')
       OR coalesce(stg.DATA_STAT_CD, 'X') <> coalesce(dv.DATA_STAT_CD, 'X')
       OR coalesce(stg.PGM_SHRT_NM, 'X') <> coalesce(dv.PGM_SHRT_NM, 'X')
       OR coalesce(stg.PGM_AR_ID, 0) <> coalesce(dv.PGM_AR_ID, 0))
THEN
UPDATE
