MERGE INTO EDV.RCD_REF_CPNT_TYPE_RS dv USING
  (SELECT DISTINCT COALESCE (trim(RCD_REF_CPNT_TYPE.RCD_REF_ID_TYPE_CD),
                       ...
                       COALESCE (RCD_REF_CPNT_TYPE.RCD_REF_CPNT_TYPE_ID,
                           ...
                           MD5 (trim(RCD_REF_CPNT_TYPE.RCD_REF_ID_TYPE_CD)||'~~'||RCD_REF_CPNT_TYPE.RCD_REF_CPNT_TYPE_ID||'~~'||trim(RCD_REF_CPNT_TYPE.DATA_STAT_CD)||'~~'||to_char(RCD_REF_CPNT_TYPE.CRE_DT, 'YYYY-MM-DD HH24:MI:SS.FF')||'~~'||trim(RCD_REF_CPNT_TYPE.CRE_USER_NM)||'~~'||to_char(RCD_REF_CPNT_TYPE.LAST_CHG_DT, 'YYYY-MM-DD HH24:MI:SS.FF')||'~~'||trim(RCD_REF_CPNT_TYPE.LAST_CHG_USER_NM)||'~~'||RCD_REF_CPNT_TYPE.EMBEDDED_DATA_STRT_BYTE_CT||'~~'||RCD_REF_CPNT_TYPE.DATA_ELE_BYTE_CT||'~~'||trim(RCD_REF_CPNT_TYPE.DPLY_IND)||'~~'||trim(RCD_REF_CPNT_TYPE.RCD_REF_CPNT_TYPE_NM)) HASH_DIF
   ...
   ORDER BY RCD_REF_CPNT_TYPE.CDC_DT) stg ON (coalesce(stg.RCD_REF_CPNT_TYPE_ID, 0) = COALESCE (dv.RCD_REF_CPNT_TYPE_ID,
                                                                                      0)) 
WHEN MATCHED
