INSERT INTO EDV.SBSD_LOC_AREA_RS (LOC_AREA_NM, LOC_AREA_CAT_NM, CTRY_DIV_NM, LOAD_DT, DATA_EFF_STRT_DT, DATA_SRC_NM, SRC_LAST_CHG_DT, SRC_LAST_CHG_USER_NM, ST_CNTY_FSA_CD, CNTY_FSA_SVC_CTR_ID, LOC_AREA_REF_ID, LOC_AREA_ID, LOC_AREA_CAT_ID, CTRY_DIV_ID, CTRY_DIV_REF_ID, DATA_EFF_END_DT, LOAD_END_DT) (
SELECT stg.*
FROM (
...
LEFT OUTER JOIN
  (SELECT OTLY_AREA_MRT_CD,
   ...
   WHERE rnum=1 ) OTLY_AREA_RS
   ON (OTLY_AREA_RS.OTLY_AREA_MRT_CD = SUBSTR(ST_CNTY_FSA_CD, 1, 2))
LEFT OUTER JOIN
  (SELECT DIST_MRT_CD,
          DIST_NM
   FROM
   ...
   WHERE rnum=1 ) DIST_RS ON (DIST_RS.DIST_MRT_CD = SUBSTR(ST_CNTY_FSA_CD, 1, 2))
LEFT OUTER JOIN
  ...
  JOIN
    (SELECT LOC_AREA_MRT_CD,
     ...
     WHERE rnum=1 ) LOC_AREA_MRT_SRC_RS ON (LOC_AREA_MRT_SRC_RS.CTRY_DIV_NM=SubQry.CTRY_DIV_NM
                                            AND LOC_AREA_MRT_SRC_RS.LOC_AREA_MRT_CD=SubQry.FSA_CNTY_CD) ) stg
  LEFT OUTER JOIN EDV.SBSD_LOC_AREA_RS dv ON (coalesce(stg.LOC_AREA_CAT_NM, 'X') = coalesce(dv.LOC_AREA_CAT_NM, 'X')
                                              AND coalesce(stg.LOC_AREA_NM, 'X') = coalesce(dv.LOC_AREA_NM, 'X')
                                              AND coalesce(stg.CTRY_DIV_NM, 'X') = coalesce(dv.CTRY_DIV_NM, 'X')
                                              AND coalesce(stg.ST_CNTY_FSA_CD, 'X')=coalesce(dv.ST_CNTY_FSA_CD, 'X')
                                              ...
                                              AND dv.LOAD_END_DT = TO_TIMESTAMP('9999-12-31', 'YYYY-MM-DD')) WHERE dv.LOC_AREA_CAT_NM IS NULL
