INSERT INTO EDV.ST_PYMT_LMT_RS (ST_NM, SBSD_PRD_NM, SBSD_PRD_ST_ID, LOAD_DT, DATA_EFF_STRT_DT, DATA_SRC_NM, DATA_STAT_CD, SRC_CRE_DT, SRC_CRE_USER_NM, SRC_LAST_CHG_DT, SRC_LAST_CHG_USER_NM, ST_FSA_CD, SCHL_PYMT_LMT_IND, SBSD_PRD_ID, CTRY_DIV_ID, CTRY_DIV_CAT_ID, CTRY_DIV_REF_ID, DATA_EFF_END_DT, LOAD_END_DT)
  (SELECT stg.*
   FROM
   ...
   LEFT JOIN EDV.ST_PYMT_LMT_RS dv ON (coalesce(stg.ST_NM, 'X') = coalesce(dv.ST_NM, 'X')
   ...
   WHERE dv.ST_NM IS NULL
     OR dv.SBSD_PRD_NM IS NULL
     OR dv.SBSD_PRD_ST_ID IS NULL )