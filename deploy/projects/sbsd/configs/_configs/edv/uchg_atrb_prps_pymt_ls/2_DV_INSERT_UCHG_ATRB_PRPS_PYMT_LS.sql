INSERT /*+ parallel (18) */ INTO EDV.UCHG_ATRB_PRPS_PYMT_LS (DIR_ATRB_PRPS_PYMT_L_ID, UCHG_ATRB_PRPS_PYMT_LOG_ID, ACCT_PGM_CD, APP_3_OWNSHP_LVL_LMT_IND, CMDY_CD, CNTY_FSA_CD, CRE_DT, CRE_USER_NM, DATA_EFF_STRT_DT, DATA_SRC_NM, DATA_STAT_CD, DEC_PRCS_NBR, HRCHL_PYMT_LMT_IND, LAST_CHG_DT, LAST_CHG_USER_NM, LOAD_DT, OVRRD_SBSD_PRD_STRT_YR, PAID_CORE_CUST_ID, PROC_RQST_REF_NBR, PRST_OPYMT_ATRB_IND, PRV_PRPS_PYMT_ACCT_PGM_CD, PRV_PRPS_PYMT_CNTY_FSA_CD, PRV_PRPS_PYMT_ID, PRV_PRPS_PYMT_SBSD_PRD_STRT_YR, PRV_PRPS_PYMT_ST_FSA_CD, PYMT_AMT, PYMT_ATRB_RQST_RSN_TXT, PYMT_ATRB_RVRS_DT, PYMT_LMT_IND, PYMT_LMT_YR, PYMT_PGM_ID, RCD_REF_PRIM_ID, RCD_REF_PRIM_ID_TYPE_CD, RCD_REF_SCND_ID, RCD_REF_SCND_ID_TYPE_CD, SBSD_PRD_STRT_YR, ST_FSA_CD, DATA_EFF_END_DT, LOAD_END_DT, HASH_DIF)
  (SELECT stg.*
   FROM
     (SELECT MD5 (upper(COALESCE (trim(UCHG_ATRB_PRPS_PYMT_LOG.PRV_PRPS_PYMT_ST_FSA_CD), '--')) ||'~~'||upper(COALESCE (trim(UCHG_ATRB_PRPS_PYMT_LOG.PRV_PRPS_PYMT_CNTY_FSA_CD), '--')) ||'~~'||upper(COALESCE (trim(UCHG_ATRB_PRPS_PYMT_LOG.PRV_PRPS_PYMT_ACCT_PGM_CD), '--')) ||'~~'||COALESCE (UCHG_ATRB_PRPS_PYMT_LOG.PAID_CORE_CUST_ID, '-1')) AS DIR_ATRB_PRPS_PYMT_L_ID,
      ...
      WHERE UCHG_ATRB_PRPS_PYMT_LOG.CDC_OPER_CD IN ('I',
                                                    'UN')
        AND date(UCHG_ATRB_PRPS_PYMT_LOG.CDC_DT) = date(TO_TIMESTAMP ('{ETL_START_TIMESTAMP}', 'YYYY-MM-DD HH24:MI:SS.FF'))
      ORDER BY UCHG_ATRB_PRPS_PYMT_LOG.CDC_DT) stg
   LEFT JOIN EDV.UCHG_ATRB_PRPS_PYMT_LS dv ON (stg.HASH_DIF = dv.HASH_DIF
