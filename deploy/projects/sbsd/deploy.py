# deploy/projects/sbsd/deploy.py
...
class SbsdNames:
    ...
    sbsd_dm_etl_state_machine: str

 


def build_names(deploy_env: str, project: str) -> SbsdNames:
    """
    ...
    return SbsdNames(
        ...
        # State machines
        edv_state_machine=f"{prefix}-EDV-Pipeline",
        sbsd_dm_etl_state_machine=f"{prefix}-DM-ETL-Pipeline",
...
def deploy(cfg: Dict[str, Any], region: str) -> Dict[str, str]:
    ...
    Deploys:
      ...
      - Step Functions: EDV pipeline + sbsd DM ETL pipeline
    ...
    project_dir = Path(__file__).resolve().parent  # .../deploy/projects/sbsd
    ...
    # --- Step Function #1: EDV pipeline (states/sbsd_stepfunction.py) ---
    edv_mod = _load_module_by_path(
        project_dir=project_dir,
        rel_path="states/sbsd_stepfunction.py",
        module_name="projects.sbsd.states.sbsd_stepfunction",
    )
    ...
    # --- Step Function #2: sbsd DM ETL pipeline (states/sbsd_dm_etl_stepfunction.py) ---
    dm_mod = _load_module_by_path(
        project_dir=project_dir,
        rel_path="states/sbsd_dm_etl_stepfunction.py",
        module_name="projects.sbsd.states.sbsd_dm_etl_stepfunction",
    )
    ...
    dm_sfn_arn = ensure_state_machine(
        sfn,
        StateMachineSpec(
            name=names.sbsd_dm_etl_state_machine,
            role_arn=sfn_role_arn,
    ...
    return {
        ...
        "state_machine_sbsd_dm_etl_arn": dm_sfn_arn,
    }
