...
def build_names(deploy_env: str, project: str) -> FpacNames:
    ...
    return FpacNames(
        ...
        jenkinshook_fn=f"{base}-JenkinsHook",
        testdatafiles_fn=f"{base}-TestDataFiles",
    )
...
def deploy(cfg: Dict[str, Any], region: str) -> Dict[str, str]:
  ...
  #  ensure_bucket_exists(s3, artifact_bucket, region)
    ...
    jenkinshook_arn = ensure_lambda(
        lam,
        LambdaSpec(
            name=names.jenkinshook_fn,
            role_arn=etl_lambda_role_arn,
            handler="lambda_function.lambda_handler",
            runtime="python3.12",
            source_dir=str(lambda_root / "JenkinsHook"),
            env=env_vars,
            layers=layers,
    ...
    testdatafiles_arn = ensure_lambda(
        lam,
        LambdaSpec(
            name=names.testdatafiles_fn,
            role_arn=etl_lambda_role_arn,
            handler="lambda_function.lambda_handler",
            runtime="python3.12",
            source_dir=str(lambda_root / "TestDataFiles"),
            env=env_vars,
            layers=layers,
            subnet_ids=subnet_ids,
            security_group_ids=security_group_ids,
    ...
    return {
        "jenkinshook_lambda_arn": jenkinshook_arn,
        "testdatafiles_lambda_arn": testdatafiles_arn,
    }
