...
WEBEQ pipeline deploy project for Glue, Lambda, and Step Functions resources.
...
- Branch naming: `<PROJECT>-<JIRA-TICKET>` (example: `WEBEQ-POPSUP-7557`)
- `WEBEQ` is the project code used in the command examples below.
...
Examples:
- FSA-FPACDEV-WEBEQ-LandingFiles
- FSA-FPACDEV-WEBEQ-Raw-DM
- FSA-FPACDEV-WEBEQ-MAIN

## Wiring Model

WEBEQ Step Functions are wired using explicit __PLACEHOLDER__ tokens in ASL files.
...
There are two deploy entry points involved for WEBEQ:

- Master deployer: deploy/deploy.py
- WEBEQ local deployer: deploy/projects/webeq/deploy.py

Relationship:

- The master deployer is a router/orchestrator for all projects.
- When called with --project-type webeq, it imports and calls projects.webeq.deploy.deploy(cfg, region).
- The WEBEQ local deployer performs all WEBEQ-specific work (Glue, Lambda, Crawler, and Step Function deployment, including ASL token replacement).

Practical usage:

- Standard entrypoint (recommended):
	python deploy.py --config config/webeq/steamdev.json --region us-east-1 --project-type webeq
- Direct WEBEQ module execution (for focused debugging):
	python projects/webeq/deploy.py --config config/webeq/steamdev.json --region us-east-1

## Config Source of Truth

Use the config files under:

- config/webeq/dev.json
- config/webeq/steamdev.json
- config/webeq/prod.json

The WEBEQ deployer reads resource settings directly from these configs, especially:
...
- project: must resolve to WEBEQ naming context
- artifacts.artifactBucket: S3 bucket for deployed Glue scripts
- artifacts.prefix: object prefix for deployment artifacts (for example webeq/)
- stepFunctions.roleArn: IAM role ARN for Step Functions state machines
- secretId: secret used by WEBEQ lambdas/Glue job arguments
...
- strparams.jobIdKeyParam: S3 object key for the job id file (for example webeq/etl-jobs/job_id.json)
...
WEBEQ Step Function ASL files under states/ now use explicit placeholder tokens that are replaced in deploy.py at deployment time.
...
Development:
python deploy.py --config config/webeq/dev.json --region us-east-1 --project-type webeq

Steamdev:
python deploy.py --config config/webeq/steamdev.json --region us-east-1 --project-type webeq

Production:
python deploy.py --config config/webeq/prod.json --region us-east-1 --project-type webeq

## Unit Tests

Run WEBEQ deploy regression tests:

python -m unittest deploy.projects.webeq.tests.test_deploy -v