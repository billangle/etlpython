...
def lambda_handler(event, context):
    ...
    if no_files:
        subject = f"No Incremental Files for WEBEQ Raw-DM on {today}{env_suffix}"
        message = (
        ...
        subject = f"WEBEQ Raw-DM: {len(display_files)} file(s) processed on {today}{env_suffix}"
        message = (
            f"WEBEQ RAW-DM Job {job_id} on {today} successfully processed "
