"""
Regression tests for deploy/projects/webeq/deploy.py.
...
# deploy/projects/webeq/tests -> deploy/projects/webeq -> deploy/projects -> deploy
_DEPLOY_ROOT = Path(__file__).resolve().parents[3]
if str(_DEPLOY_ROOT) not in sys.path:
    sys.path.insert(0, str(_DEPLOY_ROOT))

from projects.webeq import deploy as webeq_deploy  # noqa: E402
...
def _base_cfg() -> dict:
    return {
        ...
        "strparams": {
            ...
            "jobIdKeyParam": "webeq/etl-jobs/job_id.json",
            ...
            "snsArnParam": "arn:aws:sns:us-east-1:123456789012:FSA-TST-WEBEQ",
        },
        ...
        "LambdaConfig": [
            ...
            },
            {
                "RAW-DM-sns-pub-step-func-errs": {
                    ...
                    "layers": ["arn:aws:lambda:us-east-1:123456789012:layer:test:1"],
                    "environmentVariables": {
                        "SNS_ARN": "arn:aws:sns:us-east-1:123456789012:FSA-TST-WEBEQ",
                    },
                }
            },
            {
        ...
        "GlueConfig": [
            {
                "LandingFiles": {
                    ...
                    "JobParameters": {
                        ...
                        "--DestinationPrefix": "webeq/etl-jobs",
                        "--PipelineName": "webeq",
                        "--SecretId": "FSA-TST-secrets",
                        "--SourcePrefix": "webeq/dbo",
                    },
                }
            },
            {
...
class TestWebeqDeployRegression(unittest.TestCase):
    def setUp(self):
        self.cfg = _base_cfg()
    ...
    def _run_deploy(self):
        ...
        with (
            patch("projects.webeq.deploy.boto3.Session") as mock_session,
            patch("projects.webeq.deploy.ensure_bucket_exists"),
            patch("projects.webeq.deploy.ensure_glue_job", side_effect=_capture_glue),
            patch("projects.webeq.deploy.ensure_glue_database", side_effect=_capture_glue_db),
            patch("projects.webeq.deploy.ensure_lambda", side_effect=_capture_lambda),
            patch("projects.webeq.deploy.ensure_glue_crawler", side_effect=_capture_crawler),
            patch("projects.webeq.deploy.ensure_state_machine", side_effect=_capture_state),
        ):
            mock_session.return_value = MagicMock()
            result = webeq_deploy.deploy(self.cfg, self.region)
    ...
    def test_crawlers_deploy_with_env_names_recrawl_and_exclusions(self):
        ...
        self.assertIn("FSA-TST-WEBEQ", by_name)
        self.assertIn("FSA-TST-WEBEQ-cdc", by_name)

        main = by_name["FSA-TST-WEBEQ"]
        self.assertEqual(main.recrawl_behavior, "CRAWL_EVERYTHING")
        self.assertEqual(main.exclude_patterns, ["_cdc/**", "_configs/**"])
        self.assertEqual(main.s3_targets[0]["Path"], "s3://c108-tst-fpacfsa-final-zone/webeq/")

    def test_glue_databases_ensured_before_crawlers(self):
        _, captured = self._run_deploy()
        db_names = {name for name, _desc in captured["glue_dbs"]}
        self.assertEqual(db_names, {"fsa-tst-webeq", "fsa-tst-webeq-cdc"})

    def test_crawler_name_override_is_used_when_provided(self):
        _, captured = self._run_deploy()
        deployed_names = {s.name for s in captured["crawler_specs"]}
        self.assertIn("FSA-TST-WEBEQ", deployed_names)
        self.assertIn("FSA-TST-WEBEQ-cdc", deployed_names)
    ...
    def test_glue_jobs_wired_from_config(self):
        ...
        self.assertIn("FSA-TST-WEBEQ-LandingFiles", by_name)
        self.assertIn("FSA-TST-WEBEQ-Raw-DM", by_name)

        self.assertEqual(by_name["FSA-TST-WEBEQ-LandingFiles"].max_concurrency, 1)
        self.assertEqual(by_name["FSA-TST-WEBEQ-Raw-DM"].max_concurrency, 20)

    def test_glue_reference_and_python_module_paths_map_to_distinct_args(self):
        _, captured = self._run_deploy()
        by_name = {s.name: s for s in captured["glue_specs"]}
        landing_args = by_name["FSA-TST-WEBEQ-LandingFiles"].default_args
    ...
    def test_prepare_lambda_source_normalizes_hyphen_filename(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            staged_path, tmp_ctx = webeq_deploy._prepare_lambda_source(src)
            try:
                staged = Path(staged_path)
                self.assertTrue((staged / "lambda_function.py").exists())
                self.assertFalse((staged / "lambda-function.py").exists())
                sym = webeq_deploy._detect_handler_symbol(staged / "lambda_function.py")
    ...
    def test_detect_handler_symbol_raises_when_missing(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with self.assertRaises(RuntimeError):
                webeq_deploy._detect_handler_symbol(p)

    def test_all_webeq_lambdas_resolve_to_lambda_handler(self):
        lambda_root = _DEPLOY_ROOT / "projects" / "webeq" / "lambda"
        dirs = sorted([p for p in lambda_root.iterdir() if p.is_dir()])
        self.assertGreater(len(dirs), 0)

        for d in dirs:
            with self.subTest(lambda_dir=d.name):
                staged_path, tmp_ctx = webeq_deploy._prepare_lambda_source(d)
                try:
                    sym = webeq_deploy._detect_handler_symbol(Path(staged_path) / "lambda_function.py")
                    self.assertEqual(sym, "lambda_handler")
                finally:
                    if tmp_ctx is not None:
                        tmp_ctx.cleanup()
    ...
    def test_lambda_env_injections(self):
        ...
        get_inc = by_name["FSA-TST-WEBEQ-get-incremental-tables"]
        self.assertEqual(get_inc.env.get("source_folder"), "webeq")

        sns_err = by_name["FSA-TST-WEBEQ-RAW-DM-sns-pub-step-func-errs"]
        self.assertEqual(sns_err.env.get("SNS_ARN"), self.cfg["strparams"]["snsArnParam"])

        notify = by_name["FSA-TST-WEBEQ-sns-publish-validations-report"]
        self.assertEqual(notify.env.get("SNS_ARN"), self.cfg["strparams"]["snsArnParam"])

        val = by_name["FSA-TST-WEBEQ-validation-check"]
        self.assertEqual(val.env.get("SecretId"), self.cfg["secretId"])
        self.assertEqual(val.env.get("CRAWLER_NAME"), "FSA-TST-WEBEQ")
    ...
    def test_state_machine_count_and_names(self):
        ...
        self.assertEqual(
            names,
            {
                "FSA-TST-WEBEQ-Incremental-to-S3Landing",
                "FSA-TST-WEBEQ-S3Landing-to-S3Final-Raw-DM",
                "FSA-TST-WEBEQ-Process-Control-Update",
                "FSA-TST-WEBEQ-MAIN",
            },
        )
    ...
    def test_main_state_machine_child_arn_substitution(self):
        ...
        start_state = "FSA-TST-WEBEQ-Incremental-to-S3Landing"
        ...
        self.assertEqual(
            ...
            "arn:aws:states:us-east-1:123456789012:stateMachine:FSA-TST-WEBEQ-Incremental-to-S3Landing",
        )
        self.assertEqual(
            states["Update WEBEQ RAW-DM"]["Parameters"]["StateMachineArn"],
            "arn:aws:states:us-east-1:123456789012:stateMachine:FSA-TST-WEBEQ-S3Landing-to-S3Final-Raw-DM",
        )
        self.assertEqual(
            states["Update Process Control"]["Parameters"]["StateMachineArn"],
            "arn:aws:states:us-east-1:123456789012:stateMachine:FSA-TST-WEBEQ-Process-Control-Update",
        )
    ...
    def test_shortened_lambda_name_matching_in_raw_dm_state_machine(self):
        ...
        self.assertEqual(
            sns_fn,
            "arn:aws:lambda:us-east-1:123456789012:function:FSA-TST-WEBEQ-RAW-DM-sns-pub-step-func-errs",
        )

        upd_fn = states["data_ppl_job_update RAW DM"]["Parameters"]["FunctionName"]
        self.assertEqual(
            upd_fn,
            "arn:aws:lambda:us-east-1:123456789012:function:FSA-TST-WEBEQ-RAW-DM-etl-workflow-update",
        )
