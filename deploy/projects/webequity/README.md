...
WEBEQUITY pipeline deploy project for Glue, Lambda, and Step Functions resources.
...
- Branch naming: `<PROJECT>-<JIRA-TICKET>` (example: `WEBEQUITY-POPSUP-7557`)
- `WEBEQUITY` is the project code used in the command examples below.
...
Examples:
- FSA-FPACDEV-WEBEQUITY-LandingFiles
- FSA-FPACDEV-WEBEQUITY-Raw-DM
- FSA-FPACDEV-WEBEQUITY-MAIN

## Wiring Model

WEBEQUITY Step Functions are wired using explicit __PLACEHOLDER__ tokens in ASL files.
...
There are two deploy entry points involved for WEBEQUITY:

- Master deployer: deploy/deploy.py
- WEBEQUITY local deployer: deploy/projects/webequity/deploy.py

Relationship:

- The master deployer is a router/orchestrator for all projects.
- When called with --project-type webequity, it imports and calls projects.webequity.deploy.deploy(cfg, region).
- The WEBEQUITY local deployer performs all WEBEQUITY-specific work (Glue, Lambda, Crawler, and Step Function deployment, including ASL token replacement).

Practical usage:

- Standard entrypoint (recommended):
	python deploy.py --config config/webequity/steamdev.json --region us-east-1 --project-type webequity
- Direct WEBEQUITY module execution (for focused debugging):
	python projects/webequity/deploy.py --config config/webequity/steamdev.json --region us-east-1

## Config Source of Truth

Use the config files under:

- config/webequity/dev.json
- config/webequity/steamdev.json
- config/webequity/prod.json

The WEBEQUITY deployer reads resource settings directly from these configs, especially:
...
- project: must resolve to WEBEQUITY naming context
- artifacts.artifactBucket: S3 bucket for deployed Glue scripts
- artifacts.prefix: object prefix for deployment artifacts (for example webequity/)
- stepFunctions.roleArn: IAM role ARN for Step Functions state machines
- secretId: secret used by WEBEQUITY lambdas/Glue job arguments
...
- strparams.jobIdKeyParam: S3 object key for the job id file (for example webequity/etl-jobs/job_id.json)
...
WEBEQUITY Step Function ASL files under states/ now use explicit placeholder tokens that are replaced in deploy.py at deployment time.
...
Development:
python deploy.py --config config/webequity/dev.json --region us-east-1 --project-type webequity

Steamdev:
python deploy.py --config config/webequity/steamdev.json --region us-east-1 --project-type webequity

Production:
python deploy.py --config config/webequity/prod.json --region us-east-1 --project-type webequity

## Unit Tests

Run WEBEQUITY deploy regression tests:

python -m unittest deploy.projects.webequity.tests.test_deploy -v