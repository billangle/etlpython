...
def lambda_handler(event, context):
    ...
    if no_files:
        subject = f"No Incremental Files for WEBEQUITY Raw-DM on {today}{env_suffix}"
        message = (
        ...
        subject = f"WEBEQUITY Raw-DM: {len(display_files)} file(s) processed on {today}{env_suffix}"
        message = (
            f"WEBEQUITY RAW-DM Job {job_id} on {today} successfully processed "
