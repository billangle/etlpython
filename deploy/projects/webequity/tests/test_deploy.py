"""
Regression tests for deploy/projects/webequity/deploy.py.
...
# deploy/projects/webequity/tests -> deploy/projects/webequity -> deploy/projects -> deploy
_DEPLOY_ROOT = Path(__file__).resolve().parents[3]
if str(_DEPLOY_ROOT) not in sys.path:
    sys.path.insert(0, str(_DEPLOY_ROOT))

from projects.webequity import deploy as webequity_deploy  # noqa: E402
...
def _base_cfg() -> dict:
    return {
        ...
        "strparams": {
            ...
            "jobIdKeyParam": "webequity/etl-jobs/job_id.json",
            ...
            "snsArnParam": "arn:aws:sns:us-east-1:123456789012:FSA-TST-WEBEQUITY",
        },
        ...
        "LambdaConfig": [
            {
                "Job-Logging-End": {
                    ...
                    "environmentVariables": {
                        "SecretId": "FSA-TST-secrets",
                        "CRAWLER_NAME": "FSA-TST-WEBEQUITY",
                    },
                }
            },
            {
            ...
            },
            {
                "RAW-DM-sns-pub-step-func-errs": {
                    ...
                    "layers": ["arn:aws:lambda:us-east-1:123456789012:layer:test:1"],
                    "environmentVariables": {
                        "SNS_ARN": "arn:aws:sns:us-east-1:123456789012:FSA-TST-WEBEQUITY",
                    },
                }
            },
            {
        ...
        "GlueConfig": [
            {
                "LandingFiles": {
                    ...
                    "JobParameters": {
                        ...
                        "--DestinationPrefix": "webequity/etl-jobs",
                        "--PipelineName": "webequity",
                        "--SecretId": "FSA-TST-secrets",
                        "--SourcePrefix": "webequity/dbo",
                    },
                }
            },
            {
...
class TestWebequityDeployRegression(unittest.TestCase):
    def setUp(self):
        self.cfg = _base_cfg()
    ...
    def _run_deploy(self):
        ...
        with (
            patch("projects.webequity.deploy.boto3.Session") as mock_session,
            patch("projects.webequity.deploy.ensure_bucket_exists"),
            patch("projects.webequity.deploy.ensure_glue_job", side_effect=_capture_glue),
            patch("projects.webequity.deploy.ensure_glue_database", side_effect=_capture_glue_db),
            patch("projects.webequity.deploy.ensure_lambda", side_effect=_capture_lambda),
            patch("projects.webequity.deploy.ensure_glue_crawler", side_effect=_capture_crawler),
            patch("projects.webequity.deploy.ensure_state_machine", side_effect=_capture_state),
        ):
            mock_session.return_value = MagicMock()
            result = webequity_deploy.deploy(self.cfg, self.region)
    ...
    def test_crawlers_deploy_with_env_names_recrawl_and_exclusions(self):
        ...
        self.assertIn("FSA-TST-WEBEQUITY", by_name)
        self.assertIn("FSA-TST-WEBEQUITY-cdc", by_name)

        main = by_name["FSA-TST-WEBEQUITY"]
        self.assertEqual(main.recrawl_behavior, "CRAWL_EVERYTHING")
        self.assertEqual(main.exclude_patterns, ["_cdc/**", "_configs/**"])
        self.assertEqual(main.s3_targets[0]["Path"], "s3://c108-tst-fpacfsa-final-zone/webequity/")

    def test_glue_databases_ensured_before_crawlers(self):
        _, captured = self._run_deploy()
        db_names = {name for name, _desc in captured["glue_dbs"]}
        self.assertEqual(db_names, {"fsa-tst-webequity", "fsa-tst-webequity-cdc"})

    def test_crawler_name_override_is_used_when_provided(self):
        _, captured = self._run_deploy()
        deployed_names = {s.name for s in captured["crawler_specs"]}
        self.assertIn("FSA-TST-WEBEQUITY", deployed_names)
        self.assertIn("FSA-TST-WEBEQUITY-cdc", deployed_names)
    ...
    def test_glue_jobs_wired_from_config(self):
        ...
        self.assertIn("FSA-TST-WEBEQUITY-LandingFiles", by_name)
        self.assertIn("FSA-TST-WEBEQUITY-Raw-DM", by_name)

        self.assertEqual(by_name["FSA-TST-WEBEQUITY-LandingFiles"].max_concurrency, 1)
        self.assertEqual(by_name["FSA-TST-WEBEQUITY-Raw-DM"].max_concurrency, 20)

    def test_glue_reference_and_python_module_paths_map_to_distinct_args(self):
        _, captured = self._run_deploy()
        by_name = {s.name: s for s in captured["glue_specs"]}
        landing_args = by_name["FSA-TST-WEBEQUITY-LandingFiles"].default_args
    ...
    def test_prepare_lambda_source_normalizes_hyphen_filename(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            staged_path, tmp_ctx = webequity_deploy._prepare_lambda_source(src)
            try:
                staged = Path(staged_path)
                self.assertTrue((staged / "lambda_function.py").exists())
                self.assertFalse((staged / "lambda-function.py").exists())
                sym = webequity_deploy._detect_handler_symbol(staged / "lambda_function.py")
    ...
    def test_detect_handler_symbol_raises_when_missing(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with self.assertRaises(RuntimeError):
                webequity_deploy._detect_handler_symbol(p)

    def test_all_webequity_lambdas_resolve_to_lambda_handler(self):
        lambda_root = _DEPLOY_ROOT / "projects" / "webequity" / "lambda"
        dirs = sorted([p for p in lambda_root.iterdir() if p.is_dir()])
        self.assertGreater(len(dirs), 0)

        for d in dirs:
            with self.subTest(lambda_dir=d.name):
                staged_path, tmp_ctx = webequity_deploy._prepare_lambda_source(d)
                try:
                    sym = webequity_deploy._detect_handler_symbol(Path(staged_path) / "lambda_function.py")
                    self.assertEqual(sym, "lambda_handler")
                finally:
                    if tmp_ctx is not None:
                        tmp_ctx.cleanup()
    ...
    def test_lambda_env_injections(self):
        ...
        get_inc = by_name["FSA-TST-WEBEQUITY-get-incremental-tables"]
        self.assertEqual(get_inc.env.get("source_folder"), "webequity")

        sns_err = by_name["FSA-TST-WEBEQUITY-RAW-DM-sns-pub-step-func-errs"]
        self.assertEqual(sns_err.env.get("SNS_ARN"), self.cfg["strparams"]["snsArnParam"])

        notify = by_name["FSA-TST-WEBEQUITY-sns-publish-validations-report"]
        self.assertEqual(notify.env.get("SNS_ARN"), self.cfg["strparams"]["snsArnParam"])

        val = by_name["FSA-TST-WEBEQUITY-validation-check"]
        self.assertEqual(val.env.get("SecretId"), self.cfg["secretId"])
        self.assertEqual(val.env.get("CRAWLER_NAME"), "FSA-TST-WEBEQUITY")
    ...
    def test_state_machine_count_and_names(self):
        ...
        self.assertEqual(
            names,
            {
                "FSA-TST-WEBEQUITY-Incremental-to-S3Landing",
                "FSA-TST-WEBEQUITY-S3Landing-to-S3Final-Raw-DM",
                "FSA-TST-WEBEQUITY-Process-Control-Update",
                "FSA-TST-WEBEQUITY-MAIN",
            },
        )
    ...
    def test_main_state_machine_child_arn_substitution(self):
        ...
        start_state = "FSA-TST-WEBEQUITY-Incremental-to-S3Landing"
        ...
        self.assertEqual(
            ...
            "arn:aws:states:us-east-1:123456789012:stateMachine:FSA-TST-WEBEQUITY-Incremental-to-S3Landing",
        )
        self.assertEqual(
            states["Update WEBEQUITY RAW-DM"]["Parameters"]["StateMachineArn"],
            "arn:aws:states:us-east-1:123456789012:stateMachine:FSA-TST-WEBEQUITY-S3Landing-to-S3Final-Raw-DM",
        )
        self.assertEqual(
            states["Update Process Control"]["Parameters"]["StateMachineArn"],
            "arn:aws:states:us-east-1:123456789012:stateMachine:FSA-TST-WEBEQUITY-Process-Control-Update",
        )
    ...
    def test_shortened_lambda_name_matching_in_raw_dm_state_machine(self):
        ...
        self.assertEqual(
            sns_fn,
            "arn:aws:lambda:us-east-1:123456789012:function:FSA-TST-WEBEQUITY-RAW-DM-sns-pub-step-func-errs",
        )

        upd_fn = states["data_ppl_job_update RAW DM"]["Parameters"]["FunctionName"]
        self.assertEqual(
            upd_fn,
            "arn:aws:lambda:us-east-1:123456789012:function:FSA-TST-WEBEQUITY-RAW-DM-etl-workflow-update",
        )
